...
@Path("/images")
public class ImageServiceImpl extends MetadataService implements ImageService {
	...
	@Produces("text/html")
	@Path("/metadata/{id}")
	public Response updateImageMetadata(@PathParam("id") String id, HashMap<String, String> json) {
		log.trace("updateImageMetadata");
	...
	@Produces("text/html")
	@Path("/metadata/{id}")
	public Response deleteImageMetadata(@PathParam("id") String id) {
		log.trace("deleteImageMetadata");
		...
		try {
			...
			if (results.count() == 1) {
				...
				boolean success = ResourceManager.deleteResourceForId(contentPath, ResourceType.IMAGE, id);
				if (success) {
	...
	public Response find(@QueryParam("limit") Long limit,
		...
		List<DBObject> results = 
				findMetadata(MongoClient.getDB(), COLLECTION_NAME, decade, theme, limit);
