...
@Path("/video")
public class VideoServiceImpl extends MetadataService implements VideoService {
	
	private static Logger log = Logger.getLogger(VideoServiceImpl.class
			.getSimpleName());
	...
	@Produces("text/html")
	@Path("/metadata/{id}")
	public Response updateVideoMetadata(@PathParam("id") String id, HashMap<String, String> json) {
		log.trace("updateVideoMetadata");
		
		String theme = json.get("theme");
		String decade = json.get("decade");
		String metadata = json.get("metadata");
	...
	@Produces("text/html")
	@Path("/metadata/{id}")
	public Response deleteVideoMetadata(@PathParam("id") String id) {
		log.trace("deleteVideoMetadata");
		...
		try {
			...
			if (results.count() == 1) {
				...
				boolean success = ResourceManager.deleteResourceForId(contentPath, ResourceType.VIDEO, id);
				if (success) {
