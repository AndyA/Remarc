...
function initPage() {
	...
	$('#resource-file-tree').fileTree({ root: 'content/', script: 'script/connectors/jqueryFileTree.jsp' }, function(file) { 
...
function processGet(jsonResult) {
	...
	document.getElementById('info-prompt').style.display = 'block';
	...
	// Get the extra information metadata for the resource
	var infoData = jsonResult['metadata'] ? jsonResult['metadata'] : "";

	var infoDiv = "<textarea maxlength=\"500\" id=\"infodata\" class=\"meta-textarea\" rows=\"3\">" + infoData + "</textarea>";
	$("#display-info").append(infoDiv);
...
function saveResource() {
	...
	// Get the information metadata
	var infoData = document.getElementById("infodata").value.trim();
