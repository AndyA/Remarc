...
function initPage() {
	...
	$('#carousel-info-btn').click(function() {
	...
	$('#view-more-text').click(function() {
		...
		// Hide the 'More Content' button until the next time it's appropriate
		document.getElementById('view-more-text').style.display = 'none';
	});
...
function updateContentText(contentType) {
	// Change the 'More Content' button to be more relevant for the content type being displayed

	var contentBtnText = 'More Content';

	if (contentType == 'images') {
		contentBtnText = 'More Pictures';
	} else if (contentType == 'audio') {
		contentBtnText = 'More Sound Clips';
	} else if (contentType == 'video') {
		contentBtnText = 'More Film Clips';
	}

	$('#view-more-text').text(contentBtnText);
}

function showInformationDialog() {
	// Show the extra metadata for the content
	var infoElement = $("#imgCarousel").find(".active > .info-container");
	var dataInfo = infoElement.attr("data-info");

	if (dataInfo) {
		$('#info-modal').find('#infoTextField').text(dataInfo);
	} else {
		$('#info-modal').find('#infoTextField').text("No information available for this image.");
	}

	// If the content being displayed is a video, we want to pause it in the background, and start it again
	// after the dialog is closed.
	if (infoElement.is("video")) {
		infoElement.get(0).pause();
		
		// Add an event listener to restart the video
		$('#info-modal').on('hidden.bs.modal', function(e) {
			infoElement.get(0).play();
			$('#info-modal').off('hidden.bs.modal');
		});
	}
...
function showIntroductionDialog() {
	...
	if (infoElement.is("video")) {
		...
		// Add an event listener to restart the video
		$('#introduction-modal').on('hidden.bs.modal', function(e) {
			infoElement.get(0).play();
			$('#introduction-modal').off('hidden.bs.modal');
		});
	}
...
function displayContent() {
	...
	$("#thumbnailContent").empty();
		...
		$.each(metaImageJson, function(i, obj) {
			...
			var infoMetadata = obj.metadata ? " data-info=\"" + obj.metadata + "\"" : "";
			
			//The inner content of the carousel will either be an image, an image plus an audio tag, or a video
			var carouselInnerContent = "<img class=\"" + carouselImgClass +" info-container\" src=\"" + obj.imageUrl + "\" alt=\""+ obj.id +"\"" + infoMetadata + ">";
			...
			if (content == 'audio') {
				...
				videoPlayerDiv = "<video id=\"videoPlayer" + i + "\" class=\"info-container\" controls width=\"100%\" height=\"100%\" preload=\"none\"" + infoMetadata + ">" +
			...
			if (i == 0 || (i % thumbnailsPerRow) == 0) {
				...
				if (i != 0) {
					$("#thumbnails-slide-" + section).append("<div id=\"thumbnails-next-" + section + "\" class=\"container-thumbnail-content thumbnail-nav\" data-slide=\"next\"><div class=\"thumbnail-text thumbnail-next-text\"> <p>Next</p> <span class=\"glyphicon glyphicon-chevron-right\"></span></div></div>");
				}
				...
				$("#thumbnails-slide-" + section).append("<div id=\"thumbnails-prev-" + section + "\" class=\"container-thumbnail-content thumbnail-nav\" data-slide=\"prev\"><div class=\"thumbnail-text thumbnail-prev-text\"><span class=\"glyphicon glyphicon-chevron-left\"></span> <p>Prev</p></div></div>");
			}
...
function configureNewElements() {
	...
	// We want to add a listener to check if we are on the last slide. If so, we want to display the 'More Content' option
	$('#thumbnailCarousel').on('slide.bs.carousel',function(e){
		if ($(this).find('.active').index() + 1 == $(this).find('.item').length &&
			$(e.relatedTarget).index() == 0) {
			// If the next page is the last, display the 'More Content' option;
			document.getElementById('view-more-text').style.display = 'block';
		}
	});
